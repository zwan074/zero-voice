...
class FeedForward(torch.nn.Module):
    def __init__(self, dim, dim_out=None, mult=4, glu=False, dropout=0.):
        ...
        project_in = torch.nn.Sequential(
...
class ResnetBlock(BaseModule):
    ...
    def forward(self, x, mask, time_emb): # add mel style here
...
class Diffusion(BaseModule):
    ...
    def reverse_diffusion(self, z, mask, mu, n_timesteps,  stoc=False, spk=None):
        ...
        for i in range(n_timesteps):
            ...
            if stoc:  # adds stochastic term
                pred = self.estimator(xt, mask, mu, t)
                #print(pred)
                dxt_det = 0.5 * (mu - xt) - pred
    ...
    def loss_t(self, x0, mask, mu, t):
        ...
        xt, z , mean = self.forward_diffusion(x0, mask, mu, t)
        time = t.unsqueeze(-1).unsqueeze(-1)
    ...
    def forward_diffusion(self, x0, mask, mu, t):
        time = t.unsqueeze(-1).unsqueeze(-1)
        cum_noise = get_noise(time, self.beta_min, self.beta_max , cumulative=True) #* simi
        mean = x0*torch.exp(-0.5*cum_noise) + mu*(1.0 - torch.exp(-0.5*cum_noise))
        variance = 1.0 - torch.exp(-cum_noise)
        z = torch.randn(x0.shape, dtype=x0.dtype, device=x0.device, requires_grad=False)
        xt = mean + z * torch.sqrt(variance)
       
        return xt * mask, z * mask , mean
